...
import com.xsy.sys.dao.SysDeptDao;
...
@Service
public class SysDeptServiceImpl extends RenBaseServiceImpl<SysDeptDao, SysDeptEntity> implements SysDeptService {
	@Autowired
	...
	public void delete(Long id) {
		...
		if(subList.size() > 1){
			throw new GlobalException("存在子部门");
		}

		//判断部门下面是否有用户
		int count = sysUserDao.getCountByDeptId(id);
		if(count > 0){
			throw new GlobalException("部门下存在用户");
		}
	...
	private String getPidList(Long pid){
		//顶级部门，无上级部门
		if(RenConstant.DEPT_ROOT.equals(pid)){
			return RenConstant.DEPT_ROOT + "";
		}
